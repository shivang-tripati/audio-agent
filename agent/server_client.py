...
class ServerClient:
    ...
    def _on_command(self, data):
        ...
        if t == "PLAY" and self.on_play_command:
            ...
            master = data.get("masterVolume")
            branch = data.get("branchVolume")
            if master is not None and branch is not None:
                logger.info(
                    f"[Socket] VOLUME_UPDATE received: master={master}, branch={branch}")
                self.on_volume_update(master, branch)
